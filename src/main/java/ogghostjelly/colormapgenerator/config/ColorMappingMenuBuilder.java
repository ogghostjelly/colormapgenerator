...
public class ColorMappingMenuBuilder extends AbstractFieldBuilder<Optional<Block>, ColorMappingEntry, ColorMappingMenuBuilder> {
    private final Block[] valuesArray;
    private Function<Block, Text> nameProvider = null;
    private final MapColor color;

    public ColorMappingMenuBuilder(Text resetButtonKey, Text fieldNameKey, Block[] valuesArray, @Nullable Block value, MapColor color) {
    ...
    public static ColorMappingMenuBuilder start(ConfigEntryBuilder entryBuilder, Text fieldNameKey, Block[] valuesArray, @Nullable Block value, MapColor color) {
        return new ColorMappingMenuBuilder(entryBuilder.getResetButtonKey(), fieldNameKey, valuesArray, value, color);
    }

    public ColorMappingMenuBuilder setErrorSupplier(Function<Optional<Block>, Optional<Text>> errorSupplier) {
    ...
    public ColorMappingMenuBuilder setSaveConsumer(Consumer<Optional<Block>> saveConsumer) {
        return (ColorMappingMenuBuilder)super.setSaveConsumer(saveConsumer);
    }

    public ColorMappingMenuBuilder setDefaultValue(Supplier<Optional<Block>> defaultValue) {
    ...
    public ColorMappingMenuBuilder setDefaultValue(Block defaultValue) {
        return setDefaultValue(Optional.ofNullable(defaultValue));
    }

    public ColorMappingMenuBuilder setDefaultValue(Optional<Block> defaultValue) {
        return (ColorMappingMenuBuilder)super.setDefaultValue(defaultValue);
    }

    public ColorMappingMenuBuilder setTooltipSupplier(Function<Optional<Block>, Optional<Text[]>> tooltipSupplier) {
    ...
    public @NotNull ColorMappingEntry build() {
        ColorMappingEntry entry = new ColorMappingEntry(this.getFieldNameKey(), this.valuesArray, this.value.map(block -> Arrays.stream(this.valuesArray).toList().indexOf(block)).orElse(-1), this.color, this.getResetButtonKey(), this.defaultValue, this.getSaveConsumer(), (Supplier)null, this.isRequireRestart());
